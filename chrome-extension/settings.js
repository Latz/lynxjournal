...
/**
 * Build candidate WordPress base URLs to probe, in priority order.
 * Tries the entered URL first, then falls back to the site origin so that
 * entering any page URL (e.g. https://example.com/lynxjournal) still works.
 * @param {string} url - user-entered address
 * @returns {string[]} candidate base URLs, trailing slash stripped
 */
function getWpCandidateUrls(url) {
...
/**
 * Probe candidate base URLs for a WordPress REST API and return the first match.
 * @param {string[]} candidates - base URLs to try
 * @returns {Promise<{wpBase: string, resolvedOrigin: string}|null>}
 */
async function findWordPressBase(candidates) {
...
/**
 * Check whether a logged-in WordPress session cookie exists for the origin.
 * @param {string} cookieUrl - origin to check (resolved origin, so Secure cookies are included)
 * @returns {Promise<boolean>}
 */
async function hasWpLoginCookie(cookieUrl) {
...
/**
 * Fetch a WP REST nonce, then use it to retrieve and fill in the API key field.
 * Updates the status element as each step progresses.
 ...
 */
async function fetchAndFillApiKey(wpBase, status) {
...
export async function checkWpLogin(url) {
    ...
    try {
        if (!(await hasWpLoginCookie(resolvedOrigin))) {
            status.textContent = chrome.i18n.getMessage('msgNotLoggedIn');
            status.className = 'wp-login-status logged-out';
            return;
        }

        await fetchAndFillApiKey(wpBase, status);
