...
trait LynxJournal_Admin_Dashboard {
    ...
    public function handleDigestRequest(): ?array {
        if ( ! isset( $_POST['lynxjournal_create_digest'] ) ) {
            return null;
        }
        $nonce = isset( $_POST['lynxjournal_digest_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['lynxjournal_digest_nonce'] ) ) : '';
        if ( ! wp_verify_nonce( $nonce, 'lynxjournal_create_digest' ) ) {
            return null;
        }
    ...
    public function renderDashboardNotices( ?array $batch_result, ?array $digest_result ): void {
        ...
        if ( $digest_result !== null ) {
            if ( $digest_result['success'] ) {
                ...
                echo ' <a href="' . esc_url( get_permalink( $digest_result['post_id'] ) ) . '" target="_blank">' . esc_html__( 'View Post', 'lynx-journal' ) . ' →</a></p></div>';
            } else {
            ...
            <div class="inside">
                <?php if ( $unpublished_count > 0 ) : ?>
                    ...
                    <form method="post" action="">
                        <?php wp_nonce_field( 'lynxjournal_create_digest', 'lynxjournal_digest_nonce' ); ?>
                        <p>
                            <label for="digest_title"><strong><?php esc_html_e( 'Post Title', 'lynx-journal' ); ?></strong></label><br>
                            <input type="text" id="digest_title" name="digest_title" class="regular-text"
                                value="<?php
                                /* translators: %s is the current date (e.g. "April 15, 2026") */
                                echo esc_attr( sprintf( __( 'Links Digest - %s', 'lynx-journal' ), gmdate( 'F j, Y' ) ) );
                                ?>">
                        </p>
                        <input type="hidden" name="digest_as_draft" value="0">
                        <p>
                            <button type="submit" name="lynxjournal_create_digest" class="button button-primary"><?php esc_html_e( 'Publish', 'lynx-journal' ); ?></button>
                            &nbsp;
                            <button type="submit" name="lynxjournal_create_digest" value="1" onclick="this.form.elements['digest_as_draft'].value='1';" class="button"><?php esc_html_e( 'Save as Draft', 'lynx-journal' ); ?></button>
                        </p>
                    </form>
                <?php else : ?>
                    <p class="lynxjournal-muted"><?php esc_html_e( 'No pending links to publish. Add links first, then come back here to publish a digest.', 'lynx-journal' ); ?></p>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    private function renderScheduleStatusBar(): void {
        ...
        <div class="lynxjournal-schedule-status">
            ...
            <?php endif; ?>
            <a href="<?php echo $schedule_url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="lynxjournal-schedule-status-link">
    ...
    public function dashboardPage(): void {
        ...
        <div class="wrap">
            ...
            <div class="lynxjournal-stats-grid">
                <a href="<?php echo $all_links_url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="lynxjournal-stat-card lynxjournal-stat-card--link">
                ...
                <a href="<?php echo $categories_url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="lynxjournal-stat-card lynxjournal-stat-card--link">
