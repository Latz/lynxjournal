        ...
        <div className="lynxjournal-month-days">
          {monthDays.map((entry, i) => (
            ...
            <div key={entry.id ?? i} className="lynxjournal-month-day-row">
              ...
              {/* Plain <div> (not a <button>) so the NumberControl's own
                  <input> remains the only interactive element — nesting a
                  native control inside a <button> is invalid HTML and
                  ambiguous for screen readers. Activation happens when the
                  input receives focus (covers both mouse clicks and
              ...
              <div className={`lynxjournal-opt ${entry.type === 'day' ? 'lynxjournal-opt--on' : 'lynxjournal-opt--off'}`}>
                <NumberControl
                  ...
                  onFocus={() => entry.type !== 'day' && updateEntry(i, { type: 'day' })}
              ...
              <div className={`lynxjournal-opt ${entry.type === 'nth' ? 'lynxjournal-opt--on' : 'lynxjournal-opt--off'}`}>
                <SelectControl
                  ...
                  options={NTH_OPTIONS}
                  onFocus={() => entry.type !== 'nth' && updateEntry(i, { type: 'nth' })}
