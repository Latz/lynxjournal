...
describe('TriggerCondition', () => {
    it('has no accessibility violations (count mode)', async () => {
        const { container } = render(
            <TriggerCondition mode="count" value={{ count: 10, days: 7 }} onChange={noop} />
        );
    ...
    it('has no accessibility violations (age mode)', async () => {
        const { container } = render(
            <TriggerCondition mode="age" value={{ count: 10, days: 7 }} onChange={noop} />
        );
        expect(await axe(container)).toHaveNoViolations();
    });
    ...
    it('renders nothing for an unknown mode', () => {
        const { container } = render(
            <TriggerCondition mode="unknown" value={{ count: 5, days: 7 }} onChange={noop} />
        );
    ...
    describe('count mode', () => {
        it('renders a number input', () => {
            render(<TriggerCondition mode="count" value={{ count: 10, days: 7 }} onChange={noop} />);
            expect(screen.getByRole('spinbutton')).toBeInTheDocument();
        });

        it('shows singular "link" label when count is 1', () => {
            render(<TriggerCondition mode="count" value={{ count: 1, days: 7 }} onChange={noop} />);
            expect(screen.getByText('link')).toBeInTheDocument();
        });

        it('shows plural "links" label when count is > 1', () => {
            render(<TriggerCondition mode="count" value={{ count: 5, days: 7 }} onChange={noop} />);
    ...
    describe('age mode', () => {
        it('renders a number input', () => {
            render(<TriggerCondition mode="age" value={{ count: 10, days: 7 }} onChange={noop} />);
            expect(screen.getByRole('spinbutton')).toBeInTheDocument();
        });

        it('shows singular "day" label when days is 1', () => {
            render(<TriggerCondition mode="age" value={{ count: 10, days: 1 }} onChange={noop} />);
        ...
        it('defaults days to 1 when value.days is undefined', () => {
            render(<TriggerCondition mode="age" value={{ count: 10 }} onChange={noop} />);
