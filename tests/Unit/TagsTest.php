...
/**
 * Tests for LynxJournal::restGetTags() and LynxJournal::invalidateTagsCache()
 */
...
describe('LynxJournal::restGetTags()', function (): void {
    ...
    it('returns cached data and skips get_terms on a cache hit', function (): void {
        ...
        $result = $this->plugin->restGetTags(lynxjournal_make_request());
    ...
    it('fetches terms and stores them in a transient on a cache miss', function (): void {
        ...
        expect($transientKey)->toBe('lynxjournal_api_tags_list');
    ...
    it('returns the correct shape [id, name, slug, description] for each tag', function (): void {
        $terms = [
    ...
    it('returns an empty array when there are no tags', function (): void {
        Functions\when('get_transient')->justReturn(false);
...
describe('LynxJournal::invalidateTagsCache()', function (): void {
    ...
    it('deletes the correct transient keys', function (): void {
        ...
        $this->plugin->invalidateTagsCache();

        expect($deletedKeys)->toContain('lynxjournal_api_tags_list');
        expect($deletedKeys)->toContain('lynxjournal_tags_terms');
    });
});
