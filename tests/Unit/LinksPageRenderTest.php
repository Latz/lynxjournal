...
describe('LynxJournal::showLinksPage() rendering', function (): void { // NOSONAR
    ...
    it('shows a category section for each category', function (): void {
        ...
        expect($html)->toContain('lynxjournal-category-section');
        expect($html)->toContain('Tech');
    });

    it('renders the link title in the table row', function (): void {
        $link = lynxjournal_make_post(1, 'My Test Link');
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
    ...
    it('renders a URL anchor when a URL is set', function (): void {
        $link = lynxjournal_make_post(1, 'Link With URL');
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
            ->andReturn(['grouped' => ['General' => [$link]], 'max_num_pages' => 1, 'total_items' => 1]);
        Functions\when('get_post_meta')->alias(
            fn($id, $key, $single) => $key === '_lynxjournal_url' ? 'https://example.com' : ''
        );
    ...
    it('renders a dash when no URL is set', function (): void {
        $link = lynxjournal_make_post(1, 'Link Without URL');
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
            ->andReturn(['grouped' => ['General' => [$link]], 'max_num_pages' => 1, 'total_items' => 1]);
        ...
        expect($html)->toContain('lynxjournal-status-unpublished');
        expect($html)->not->toContain('target="_blank"');
    });

    it('defaults publish_status to unpublished when empty', function (): void {
        $link = lynxjournal_make_post(1, 'New Link');
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
            ->andReturn(['grouped' => ['General' => [$link]], 'max_num_pages' => 1, 'total_items' => 1]);
    ...
    it('shows the published date when set', function (): void {
        ...
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
            ->andReturn(['grouped' => ['General' => [$link]], 'max_num_pages' => 1, 'total_items' => 1]);
        Functions\when('get_post_meta')->alias(
            fn($id, $key, $single) => $key === '_lynxjournal_published_date' ? '2026-01-15' : ''
        );
    ...
    it('truncates long URLs at 50 characters', function (): void {
        ...
        $link = lynxjournal_make_post(1, 'Long URL Link');
        $this->plugin->shouldReceive('getLinksGroupedByCategory')
            ->andReturn(['grouped' => ['General' => [$link]], 'max_num_pages' => 1, 'total_items' => 1]);
        Functions\when('get_post_meta')->alias(
            fn($id, $key, $single) => $key === '_lynxjournal_url' ? $longUrl : ''
        );
